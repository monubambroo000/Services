...
function Run () {
    ...
    $conflictingContainerIds = $(docker ps | select-string -pattern ":8088->" | foreach { Write-Output $_.Line.split()[0] })

    if ($conflictingContainerIds) {
        $conflictingContainerIds = $conflictingContainerIds -Join ' '
        Write-Host "Stopping conflicting containers using port 8088"
...
function GetUrl () {
    if ([System.String]::IsNullOrWhiteSpace($Machine)) {
        $launchURL = [System.UriBuilder]"http://localhost:8088"
    }
    else {
